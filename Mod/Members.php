...
<?php
if ($_GET['act']=="View") {?>
<title><?php echo $BoardName?> <?php echo $TitleLine ?> <?php echo $adminlang2['mod tools']; ?> <?php echo $TitleLine ?> <?php echo $adminlang2['member tools']; ?></title>
<?php }
...
<?php /* ?><label for="Group"><?php echo $adminlang2['pick user group for member']; ?></label><br />
<select size="1" name="NewUserGroup" id="Group" class="Menu"><option value="<?php echo $OldUserGroup ?>" selected="selected" class="Menu"><?php echo $adminlang2['keep group']; ?></option><option value="Banned"><?php echo $adminlang2['banned']; ?></option><option value="Guest"><?php echo $adminlang2['guest']; ?></option>
<option value="Member"><?php echo $adminlang2['member']; ?><option value="Moderator"><?php echo $adminlang2['moderator']; ?></option><option value="Admin"><?php echo $adminlang2['admin']; ?></option></select><?php */ ?><br />
<input type="hidden" class="HiddenTextBox" style="display: none;" name="NewUserGroup" value="<?php echo $OldUserGroup ?>" />
...
<!--<label for="SetSkin">Change Board Skin to:<br /></label>
...
$query="UPDATE ".$TablePreFix."Members SET Email='".$_POST['NewUserEmail']."',Group='".$_POST['NewUserGroup']."',`WarnLevel`='".$NewWarnLevel."',Interests='".$NewInterests."',Title='".$NewUserTitle."',LastActive=".$_POST['NewLastActive'].",Signature='".$NewUserSignature."',Avatar='".$_POST['NewUserAvatar']."',`AvatarSize`='".$_POST['NewUserAvatarSize']."',Website='".$_POST['NewUserWebsite']."',IP='".$_POST['NewUserIP']."' WHERE id=".$_POST['OldUserid']."";
mysql_query($query);
echo '<meta http-equiv="Refresh" Content="0; URL=index.php?act=View">'; }
/*if ($_POST['act']=="Delete") {
	if ($_POST['IDN']==1) { echo "You cant delete the Root Admin"; exit(); }
	if ($_POST['IDN']==2) { echo "You cant delete the Root Guest"; exit(); }
